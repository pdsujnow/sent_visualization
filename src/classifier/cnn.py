...
class CNN(KerasClassifier):
    ...
    def __call__(self, maxlen = 10,
        ...
        model = Sequential()
        model.add(MyEmbedding(vocabulary_size, embedding_dims, input_length=maxlen))
        #model.add(Dropout(drop_out_prob))
        model.add(self.convLayer(maxlen, embedding_dims, nb_filter, filter_length))
